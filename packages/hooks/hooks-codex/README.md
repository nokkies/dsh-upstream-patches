...
The config is parsed **once** at load. `configPath` is **process-level** — a relative path resolves against the process launch cwd at load time, not per-session (`TODO(per-session-hook-config)`). A read/parse failure is contained (logs + registers nothing); an invalid regex matcher on an event that consumes matchers is one such failure and reports its pattern and event. Only sync `type: 'command'` hooks run — a non-command or `async: true` hook is parsed-and-skipped with a warning. A hook accepts `timeout` or the `timeoutSec` alias; one that sets neither runs under the protocol's reference default (`DEFAULT_HOOK_TIMEOUT_MS` from `dsh-hook-protocol`, 10 minutes). A timeout the shell executor cannot run with is refused rather than accepted: `defaultTimeoutMs` must be a positive finite number of milliseconds or the plugin fails to load, and either per-hook spelling must be a positive finite number of seconds or the config is rejected with its event, like an invalid matcher. This is a security property, not tidiness -- a non-positive timeout makes every invocation of that hook a non-blocking infrastructure error, so the merged outcome is "no decision" and a `PreToolUse` deny is skipped while the tool runs. Fractional values are accepted, because the executors assert positive-and-finite rather than integral. Events outside the five bridge-supported points are dropped at parse.
