...
- **`matcherDiagnostic(matcher, mode)` / `matchesMatcher(matcher, query, mode)`**：缺失、`''` 或 `'*'` 时匹配全部；`claude` mode 将纯 `[A-Za-z0-9_|]+` pattern 视为字面量（管道符 = 精确匹配交替，且比较时**忽略大小写**），其他 pattern 视为正则；`codex` mode 始终使用未锚定正则。忽略大小写只作用于字面量路径：Claude 的 `hooks.json` 词表使用 PascalCase（`Bash`、`Read`），而本 harness 注册的是小写工具名，因此区分大小写的比较会静默丢弃所有从 Claude Code 迁移过来的 matcher——其中包括 `PreToolUse` 的拒绝规则。这不会把字面量放宽成子串匹配（`Bash` 仍然不匹配 `BashOutput`），两种方言的正则也都保持区分大小写，因为正则正是调用方用来精确控制的手段。桥接解析器会丢弃没有 matcher 匹配对象的事件所带的 matcher 字段，再用 `matcherDiagnostic` 拒绝事件实际使用的无效正则，并在注册任何钩子之前给出稳定诊断。运行时谓词仍会将无效 pattern 隔离为不匹配，因此直接调用本库不会向 agent loop（智能体循环）抛异常。
- **`timeoutDiagnostic(timeoutSec)`**：`matcherDiagnostic` 在 timeout 上的对应物，承担同样的解析期职责：值可运行时返回 `undefined`，否则返回指明该值的稳定诊断。要求正且**有限**，与执行器自身的契约一致，而非 `stderrSummaryMaxChars` 的正整数规则，因此 `1.5` 秒这样的小数 timeout 会被接受。桥接在注册任何 hook 之前用它拒绝错误的每 hook `timeout`，并在加载时断言自身的 `defaultTimeoutMs`。故意做成响亮失败：执行器拒绝的 timeout 会使该 hook 的每一次调用都变成非阻塞错误，而被跳过的 `PreToolUse` 拒绝会让工具照常运行。
