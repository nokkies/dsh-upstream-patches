...
- **`matcherDiagnostic(matcher, mode)` / `matchesMatcher(matcher, query, mode)`** — match-all on absent/`''`/`'*'`; `claude` mode treats a pure `[A-Za-z0-9_|]+` pattern as a literal (pipe = exact-match alternation, compared **without regard to case**) and anything else as a regex; `codex` mode is always an unanchored regex. Case-insensitivity is confined to the literal path because Claude's `hooks.json` vocabulary is PascalCase (`Bash`, `Read`) while this harness registers lowercase tool names, so an exact compare would silently drop every matcher migrated from Claude Code — a `PreToolUse` deny among them. It does not widen a literal into a substring match (`Bash` still rejects `BashOutput`), and regexes stay case-sensitive in both dialects, since a regex is where a caller asks for exact control. Bridge parsers discard matcher fields for events without matcher subjects, then use `matcherDiagnostic` to reject an invalid consumed regex with a stable diagnostic before registering any hooks. The runtime predicate still contains an invalid pattern as a non-match, so a direct library caller cannot throw into the agent loop.
- **`timeoutDiagnostic(timeoutSec)`** — the timeout counterpart to `matcherDiagnostic`, for the same parse-time role: `undefined` when the value is runnable, otherwise a stable diagnostic naming it. Positive **finite**, matching the executors' own contract rather than `stderrSummaryMaxChars`'s positive-integer rule, so a fractional `1.5`-second timeout is accepted. Bridges reject a bad per-hook `timeout` with it before registering any hooks, and assert their own `defaultTimeoutMs` at load. Loud on purpose: a timeout the executor refuses makes every invocation of that hook a non-blocking error, and a skipped `PreToolUse` deny runs the tool.
